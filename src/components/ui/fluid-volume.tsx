...
export interface FluidVolumeProps {
  ...
  /** Fold frequency. */
  waveFactor?: number;
  /** Tonal crush exponent — lower is lighter. */
  crush?: number;
  /** vUv.y at which the field becomes fully opaque. */
  alphaBottom?: number;
  className?: string;
  style?: CSSProperties;
...
function makeNoiseTexture(): THREE.DataTexture {
  ...
  const sample = (g: Float32Array, x: number, y: number): number => {
    ...
    const at = (cx: number, cy: number) =>
      g[
        (((cy % period) + period) % period) * period +
          (((cx % period) + period) % period)
      ];
    const a = at(ix, iy);
    const b = at(ix + 1, iy);
    const c = at(ix, iy + 1);
...
export function FluidVolume({
  ...
  if (!webGLSupported) {
    return (
      <div className={className} style={{ background: baseColor, ...style }} />
    );
  }
  return (
    <div
