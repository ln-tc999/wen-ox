...
function WagmiAndWalletProviders({ children }: ProvidersProps) {
  ...
  useEffect(() => {
    ...
    fetch("/api/wallet-config", {
      ...
      .then((data) => {
        if (!data?.projectId) {
          // Fallback: use dummy project ID for development
          console.warn("No PROJECT_ID found, using development mode");
          setConfig(createWagmiConfig("demo-project-id-for-dev"));
          return;
        }
        setConfig(createWagmiConfig(data.projectId));
      })
      .catch(() => {
        // On error, still set config with dummy ID for development
        console.warn("Failed to fetch wallet config, using development mode");
        setConfig(createWagmiConfig("demo-project-id-for-dev"));
      });
